  ...
  while (nr_replied < (int) nr_pending) {
    ...
    if (pfd.revents & POLLIN) {
      ...
      while (true) {
        ...
        bool result = ParseBuffer(inbuf + instart, &len, nr_replied);
        if (!result) {
          break;
        }
        instart += len;
...
bool BaseClient::ParseBuffer(uint8_t *buf, uint32_t *in_len, int nr_replied)
{
  ...
  if (!pending[nr_replied]->HandleResponse(buf  + sizeof(SunRpcAcceptHeader), &len)) {
    return false;
  }
  if (log_enabled)
