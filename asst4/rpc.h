...
class BaseProcedure {
 ...
 protected:
  ...
  virtual bool DecodeAndExecute(uint8_t *in_bytes, uint32_t *in_len,
                                uint8_t *out_bytes, uint32_t *out_len) = 0;
};
...
class BaseResult {
 ...
 protected:
  bool ready = false;
  bool error = false;
  virtual bool HandleResponse(uint8_t *in_bytes, uint32_t *in_len) = 0;
  virtual ~BaseResult() {}
 public:
...
class BaseClient {
 ...
 private:
  bool ParseBuffer(uint8_t *inbytes, uint32_t *in_len, int nr_replied);
};
