...
class MetaTable(type):
            ...
            if type(cls.field[fieldIndex]) == field.Coordinate:
                ...
                for colIndex, colName in enumerate(MetaTable.tables[cascTableName].column):
                    ...
                    if type(colType) is field.Coordinate:
                        cascValues.append(
                            (cascVal[resultIndex], cascVal[resultIndex+1]))
                        resultIndex += 1
                    else:
    ...
    def filter(cls, db, **kwarg):
        ...
        if type(value) is tuple:
            pks = db.scan(cls.__name__, op, column_name=colName +
                          '_lat', value=value[0])
            pks2 = db.scan(cls.__name__, op,
                           column_name=colName+'_lon', value=value[1])
            finalPks = list(set(pks) & set(pks2))
        elif type(value) is datetime:
            finalPks = db.scan(
                cls.__name__, op, column_name=colName, value=value.timestamp())
        else:
            finalPks = db.scan(
                cls.__name__, op, column_name=colName, value=value)
        for pk in finalPks:
            row = cls.get(db, pk)
            result.append(row)
        return result
